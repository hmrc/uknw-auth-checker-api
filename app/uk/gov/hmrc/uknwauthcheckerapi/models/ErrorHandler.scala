...
trait ErrorHandler extends Logging {
  ...
  implicit val IntegrationFrameworkErrorConverter: Converter[IntegrationFrameworkError] =
    {
      case IntegrationFrameworkError.BadGateway(cause, statusCode) => ResponseError.badGateway(cause, statusCode)
      case IntegrationFrameworkError.InternalUnexpectedError(cause) =>
